...
public class HugeOrangeGlowshroomFeature extends HugeMushroomFeature
{
				...
				if(!isXCorner || !isZCorner)
				{
					...
					if(!world.getBlockState(mutable).isOpaqueFullCube(world, mutable))
					{
						...
						if(isMiddle)
							...
							world.setBlockState(mutable.down(), config.capProvider.getBlockState(random, start).with(MushroomBlock.DOWN, false), 3);
					}
				}
			}
	...
	{
		if(world.isAir(pos) || world.getFluidState(pos).isIn(FluidTags.WATER)) world.setBlockState(pos, state, 3);
	}
